...
def _extract_pdp_xy(pd_result):
    ...
    if hasattr(pd_result, "average"):
        ys = pd_result.average[0]
        xs = None
        if hasattr(pd_result, "values") and pd_result.values is not None:
            xs = pd_result.values[0]
        elif hasattr(pd_result, "grid_values") and pd_result.grid_values is not None:
            xs = pd_result.grid_values[0]
        if xs is None:
            raise ValueError("Partial dependence no expuso 'values' ni 'grid_values'.")
        return xs, ys
...
alpha_ridge = st.sidebar.slider("Ridge alpha", 0.0001, 10.0, 1.0)
alpha_lasso = st.sidebar.slider("Lasso alpha", 0.0001, 10.0, 0.1)
alpha_en = st.sidebar.slider("ElasticNet alpha", 0.0001, 10.0, 0.5)
l1_ratio_en = st.sidebar.slider("ElasticNet l1_ratio", 0.0, 1.0, 0.5)
svr_C = st.sidebar.slider("SVR C", 0.1, 100.0, 10.0)
svr_epsilon = st.sidebar.slider("SVR epsilon", 0.01, 1.0, 0.1)
